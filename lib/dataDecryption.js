...
module.exports = class DataEncryption extends ServiceBase {
  ...
  async run () {
    ...
    const secrets = DataEncryption._decryptSecretKeyRSA(this.secrets, this.rsaPrivateKey)
    return {
      decryptedFilePath: await DataEncryption._decryption(this.encryptedFilePath, secrets, this.decryptedFilePath),
  ...
  static async _decryption (encryptedFilePath, secrets, decryptedFilePath) {
    return new Promise(function (resolve, reject) {
      ...
      const decrypt = crypto.createDecipheriv(secrets.alogorithm, Buffer.from(secrets.secretKey, 'hex'), Buffer.from(secrets.iV, 'hex'))
      decrypt.setAutoPadding(secrets.padding)
      try {
        readStream.pipe(decrypt).pipe(writeStream)
        writeStream.on('finish', () => { resolve(writeStream.path) })
